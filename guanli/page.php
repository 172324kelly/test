...
try{
	...
	if(!isset($_GET['id']) || !filter_var($_GET['id'],FILTER_VALIDATE_INT,array('min_range'=>1))){//222222.这里本来少一个）右括号
		throw new Exception('An invalid page ID was provide to this page.');	
	}

	//Fetch the page from the database:
	$q = 'SELECT id, creatorld, title, content, DATE_FORMAT(dateAdded, "%e %M %Y") AS dateAdded FROM pages WHERE id=:id';
	$stmt = $pdo->prepare($q);
